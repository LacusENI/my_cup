...
WAVES_DIR  = waves
LOGS_DIR   = logs
INC_PATH   = lib rtl/core/include sim/tb/include 
...
build-%: $(F_DIR)/%.f
	...
	@iverilog $(BUILD_FLAGS) -o $(TARGET_DIR)/$* -f $^ $(addprefix -I,$(INC_PATH)) >> $(BUILD_LOG) 2>&1 && echo "OK" | tee -a $(BUILD_LOG) || (echo "FAILED"; exit 1)
